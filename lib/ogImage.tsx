...
export async function generateListOgImage(label: string, title: string, subtitle: string) {
  ...
  return new ImageResponse(
    (
      <div
        style={{
          ...
          background: "linear-gradient(180deg, #050c18 0%, #0a1628 35%, #0f2035 65%, #152530 100%)",
          position: "relative",
          overflow: "hidden",
        }}
      >
        ...
        >
          <p style={{ color: "#7ecfb3", fontSize: "20px", letterSpacing: "0.2em", marginBottom: "16px", fontFamily }}>
            {label}
          </p>
          <h1
            style={{
          ...
          <p style={{ color: "#a8c4d4", fontSize: "22px", lineHeight: 1.7, fontFamily }}>
            {subtitle}
          </p>
        </div>
    ...
    ),
    {
      width: 1200,
      height: 630,
      ...(fontData ? { fonts: [{ name: "NotoSansJP", data: fontData, weight: 700 }] } : {}),
    }
  )
}
