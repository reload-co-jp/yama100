...
export async function generateMetadata({
  ...
  const canonicalPath = `/mountains_minor12/${mountain.id}/`
  const ogTitle = `${mountain.name}（${mountain.elevation.toLocaleString()}m）- マイナー12名山`
...
export default async function MountainMinor12Page({
  ...
  const jsonLd = {
    ...
    name: mountain.name,
    description: mountain.description,
    url: `/mountains_minor12/${mountain.id}/`,
    geo: {
      "@type": "GeoCoordinates",
    ...
    <div style={{ maxWidth: "720px", margin: "0 auto" }}>
      ...
      >
        ...
        <div style={{ padding: "24px" }}>
          ...
          <DetailMap mountain={mountain} storageKey="yama_minor12" />
        </div>
      </div>
      ...
      >
        {prev ? (
          <Link
            href={`/mountains_minor12/${prev.id}/`}
            style={{
              background: "#2a2a2a",
              borderRadius: "8px",
              color: "#ccc",
        ...
        {next ? (
          <Link
            href={`/mountains_minor12/${next.id}/`}
            style={{
              background: "#2a2a2a",
              borderRadius: "8px",
              color: "#ccc",
