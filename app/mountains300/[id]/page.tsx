...
import MountainDetailClient300 from "../../../components/MountainDetailClient300"
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  const canonicalPath = `/mountains300/${mountain.id}/`
  const ogTitle = `${mountain.name}（${mountain.elevation.toLocaleString()}m）- 日本三百名山`
...
export default async function Mountain300Page({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const mountain = mountains.find((m) => m.id === Number(id))
  ...
  return (
    <div style={{ maxWidth: "720px", margin: "0 auto" }}>
      ...
      >
        <MountainDetailClient300 mountain={mountain} />
