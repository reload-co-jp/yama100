...
// Module-level store so React can subscribe to external (localStorage) state
...
function writeChecked(next: Set<number>) {
  _cache = next
  localStorage.setItem("yama100", JSON.stringify({ checked: [...next] }))
...
export default function MountainDetailClient({ mountain }: Props) {
  // getServerSnapshot returns EMPTY → server and initial client render match (no hydration mismatch)
  // getSnapshot reads localStorage → client updates after hydration
